...
template <typename T>
linked_list<T>::linked_list(const size_t &size) : head(nullptr), tail(nullptr), length(0)
{
...
template <typename T>
linked_list<T>::linked_list(const T *data, const size_t &size) : head(nullptr), tail(nullptr), length(0)
{
    for (int i = 0; i < size; i++)
    {
